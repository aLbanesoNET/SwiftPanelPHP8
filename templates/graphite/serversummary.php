...
<section class="graphite-powerbar">
	<div class="graphite-powerbar-id">
		<span class="graphite-dot graphite-dot-<?= $online === 'Started' ? 'ok' : ($online === 'Pending' ? 'warn' : 'off') ?>"></span>
		<h1><?= htmlspecialchars($srv['name'] ?? ('Server #' . $sid)) ?></h1>
		<span class="graphite-pill graphite-pill-<?= $statusCls ?>"><?= htmlspecialchars($status) ?></span>
		<span class="graphite-pill graphite-pill-mono">#<?= $sid ?></span>
	</div>

	<div class="graphite-powerbar-act">
		<?php if ($status === 'Active' && $online === 'Stopped'): ?>
			<a class="graphite-btn graphite-btn-go" href="servermanage.php?task=start&amp;serverid=<?= $sid ?>">&#9658; Start</a>
			<?php if (!empty($srv['installdir'])): ?>
				<a class="graphite-btn graphite-btn-ghost" href="#"
		...
		<?php elseif ($status === 'Active' && $online === 'Started'): ?>
			<a class="graphite-btn" href="servermanage.php?task=restart&amp;serverid=<?= $sid ?>">&#8635; Restart</a>
			<a class="graphite-btn graphite-btn-stop" href="servermanage.php?task=stop&amp;serverid=<?= $sid ?>">&#9632; Stop</a>
		<?php endif; ?>

		<a class="graphite-btn graphite-btn-ghost" href="serverplayers.php?id=<?= $sid ?>">Players</a>
		<?php if ($isOwner): ?>
			<?php if (!empty($srv['webftp'])): ?>
				<a class="graphite-btn graphite-btn-ghost" href="serverftp.php?id=<?= $sid ?>">Web FTP</a>
			<?php endif; ?>
			<a class="graphite-btn graphite-btn-ghost" href="serverschedule.php?id=<?= $sid ?>">Schedules</a>
			<a class="graphite-btn graphite-btn-ghost" href="serverbackup.php?id=<?= $sid ?>">Backups</a>
			<a class="graphite-btn graphite-btn-ghost" href="serversubusers.php?id=<?= $sid ?>">Share</a>
		<?php endif; ?>
	</div>
</section>
<?php if (!$isOwner): ?>
	<div class="graphite-note graphite-note-ok"><strong>Shared with you</strong><span>You can view this server, use the console and start/stop it.</span></div>
<?php endif; ?>

<div class="graphite-grid graphite-grid-2">
	<div class="graphite-col">
		<div class="graphite-card">
			<div class="graphite-card-head"><h2>Server information</h2></div>
			<dl class="graphite-dl">
				...
				<?php endif; ?>
				<dt>Status</dt><dd><span class="graphite-pill graphite-pill-<?= $statusCls ?>"><?= htmlspecialchars($status) ?></span></dd>
			</dl>
		</div>

		<form method="post" action="serverprocess.php" class="graphite-card">
			<input type="hidden" name="task" value="serveredit">
			<input type="hidden" name="serverid" value="<?= $sid ?>">

			<div class="graphite-card-head"><h2>Server configuration</h2></div>
			<dl class="graphite-dl">
	...
	<div class="graphite-col">
		<?php if (!empty($spark)): ?>
			<div class="graphite-card">
				<div class="graphite-card-head"><h2>Players &mdash; last 24h</h2></div>
				<div class="graphite-spark-wrap"><?= $spark ?></div>
			</div>
		<?php endif; ?>
		...
		<div class="graphite-card">
			<div class="graphite-card-head">
				<h2>Server status</h2>
				<a class="graphite-card-link" href="#" onclick="window.location.reload();return false;">Refresh</a>
			</div>
			<dl class="graphite-dl">
				<dt>State</dt><dd><span class="graphite-pill graphite-pill-<?= $onlineCls ?>"><?= htmlspecialchars($online) ?></span></dd>
		...
		<?php if (!empty($srv['showftp']) && !empty($srv['ip']) && $isOwner): ?>
			<div class="graphite-card">
				<div class="graphite-card-head"><h2>FTP details</h2></div>
				<dl class="graphite-dl">
		...
		<?php if (!empty($srv['ipid']) && $isOwner): ?>
			...
			<div class="graphite-card">
				<div class="graphite-card-head">
					<h2>FastDL</h2>
					<span class="graphite-pill <?= $fdlToken !== '' ? 'graphite-pill-ok' : 'graphite-pill-mono' ?>"><?= $fdlToken !== '' ? 'On' : 'Off' ?></span>
				</div>
				<?php if ($fdlToken !== ''): ?>
					...
					<div class="graphite-form-actions">
						<a class="graphite-btn graphite-btn-stop" href="fastdlprocess.php?task=disable&amp;serverid=<?= $sid ?>" onclick="return confirm('Turn off FastDL for this server?');">Disable FastDL</a>
					</div>
				<?php else: ?>
					<p>Host your server's downloadable content (maps, models, sounds) over HTTP so players join faster.</p>
					<div class="graphite-form-actions">
						<a class="graphite-btn" href="fastdlprocess.php?task=enable&amp;serverid=<?= $sid ?>">Enable FastDL</a>
					</div>
				<?php endif; ?>
			</div>
		<?php endif; ?>
...
if ($cInstalled):
?>
<div class="graphite-card graphite-console-card">
	<div class="graphite-card-head">
		<h2>Console</h2>
		<a class="graphite-card-link" href="serverlogdownload.php?id=<?= $sid ?>">Download log</a>
		<label class="graphite-check graphite-check-sm" style="margin-left:12px;"><input type="checkbox" id="swConsoleAuto"> Auto-refresh</label>
	</div>
	<pre id="swConsoleOut" class="swconsole">Connecting&hellip;</pre>
	<?php if ($cOnline): ?>
	<div class="graphite-quickcmds">
		<?php foreach (['status', 'stats', 'users'] as $qc): ?>
			<button type="button" class="graphite-btn graphite-btn-ghost" onclick="swConsoleQuick('<?= $qc ?>')"><?= $qc ?></button>
		<?php endforeach; ?>
	</div>
	<?php endif; ?>
	<form onsubmit="return swConsoleSend();" class="graphite-console-form">
		<input type="text" id="swConsoleCmd" autocomplete="off" placeholder="command, e.g. status" <?= $cOnline ? '' : 'disabled' ?>>
		<button type="submit" class="graphite-btn" <?= $cOnline ? '' : 'disabled' ?>>Send</button>
		<button type="button" class="graphite-btn graphite-btn-ghost" onclick="swConsoleRefresh()">Refresh</button>
	</form>
</div>
